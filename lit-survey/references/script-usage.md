...
脚本是可选工具，主流程见 [SKILL.md](../SKILL.md)。已有的学术搜索、论文阅读工具也可以承担这些步骤。

## 路径与依赖

命令中的 `<skill-dir>` 是包含 `SKILL.md` 的实际目录，`<workdir>` 是本次调研的输出目录。运行前替换所有尖括号占位符，并保留路径外的引号。不要假设运行环境提供同名环境变量或保留前一次命令的工作目录。

使用环境中可用的 Python 命令；下文写作 `python`，也可换成 `python3`。脚本需要 Python 3.9+，网络请求使用标准库。PDF 文本抽取还需要 PyMuPDF：

```sh
python -m pip install pymupdf
```

Semantic Scholar key 可以通过 `--s2-key` 或 `S2_API_KEY` 提供；OpenAlex 联系邮箱可以通过 `--mailto` 或 `OPENALEX_MAILTO` 提供。请求会缓存和退避重试，实际可用性取决于服务配额。
...
这个检查器目前仍按固定格式要求引用数、页码和 gap 关键词，不能完全覆盖主流程允许的网页来源、缺失元数据或简版报告。逐项核对检查结果，修正实际错误；不能取得的信息据实说明，不为通过检查补写数字、页码或研究空白。检查器也不验证研究者阅读状态、引文语义或结论真假。
